...
// STL에는 list가 제공됩니다.
// STL을 사용할 수 있는 상황이라면 직접 연결 리스트를 구현하는 것보다
// STL list를 사용하는 것이 훨씬 편리합니다.
// vector 사용법처럼, 예시를 통해 STL list의 사용법을 이해해보겠습니다.

// STL list에서 제공하는 함수들은 다음과 같습니다:
// - push_back: O(1) - 리스트의 끝에 원소를 추가
// - pop_back: O(1) - 리스트의 끝에서 원소를 제거
// - push_front: O(1) - 리스트의 앞에 원소를 추가
// - pop_front: O(1) - 리스트의 앞에서 원소를 제거

// '번지'라는 개념을 사용했던 이전 구현과 달리,
// STL list에서는 iterator가 주소 역할을 합니다.
// 예를 들어, list<int>::iterator t = L.begin(); 대신
// C++11 이상에서는 auto t = L.begin(); 으로 간편하게 사용할 수 있습니다.

// erase 함수는 제거한 다음 원소의 위치를 반환합니다.
// 순회할 때에는 C++11 이상에서는 range-based for loop을 사용할 수 있지만,
// C++11 미만에서는 iterator를 사용한 불편한 방법을 사용해야 합니다.

// 야매 연결 리스트에서는 0번지를 더미 노드로 사용했지만,
// STL list에서는 제일 뒤의 원소가 더미 노드입니다.
// 따라서, L.begin()은 제일 앞에 있는 원소를 가리키지만
// L.end()는 제일 뒤에 있는 원소의 한 칸 뒤를 가리킨다는 점에 주의해야 합니다.
